...
bool FileIO::write(const std::string& filename,
  ...
  try {
    ...
    binary_data_stream.write(
      reinterpret_cast<const char*>(p_data), size);
...
bool FileIO::read(const std::string& filename,
  ...
  if (binary_data_stream.is_open()) {
    ...
    binary_data_stream.read(
      reinterpret_cast<char*>(input_data.data()), input_size);
...
void FileIO::write_hex_buffer(std::ofstream& tensor_file,
